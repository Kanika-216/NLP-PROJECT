...
def analyze():
    ...
    # 4. Handle "Readability Mode" Action
    elif action == 'readability':
        # Create a report based on the stats calculated in process_text
        score = results.get('readability', {}).get('score', 0)
        level = results.get('readability', {}).get('level', 'Unknown')
